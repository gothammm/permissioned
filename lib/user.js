...
class User {
  ...
  assignByRoleName(roleName) {
    ...
    let getRoleByName = Bluebird.coroutine(storage.get.bind(storage));

    return getRoleByName(_self.containers.ROLE, {
      name: roleName
    }).then(role => {
      if (!role) {
        return Bluebird.reject(`Role name "${roleName}" not found`);
      }
      return _self.assign(role._id);
    });
  }
  assign(roleName) {
    let _self = this;
